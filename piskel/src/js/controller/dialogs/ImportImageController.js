(function () {
  ...
  ns.ImportImageController = function (piskelController) {
    ...
    this.file_ = null;
    this.pController = piskelController;
  };
  ...
  ns.ImportImageController.prototype.importImageToPiskel_ = function () {
    ...
    if (image) {
       ...
       if (this.getImportType_() === 'current') {
         var cs1 = 'You are about to add a new frame to Piskel.\n\n';
         var cs2 = 'If this is the first image you are importing, all proceeding images will be resized.';
         confirmation = window.confirm(cs1 + cs2);
       } else {
         confirmation = window.confirm('You are about to create a new Piskel, unsaved changes will be lost.');       
       }
      ...
      if (confirmation) {
        ...
        gifLoader.load({
          ...
          error: function () {
            ...
            } else if (this.getImportType_() === 'current') {
                // Single image to current frame    		 
              this.createPiskelFromImagesNewFrame_([image], resizeW, resizeH);        		       
            } else {
              // Spritesheet
  ...
  ns.ImportImageController.prototype.createPiskelFromImagesNewFrame_ = function (images, w, h) { 
    var framesInfo = pskl.app.getFrameInformation();
       
    //Set the global width and height for all images if this is the first image imported into the Piskel
    if (framesInfo.getFrameWidth() === 0 && framesInfo.getFrameHeight() === 0) {
      framesInfo.setFrameHeight(h);
      framesInfo.setFrameWidth(w);
    } 

	//resize every image to the global frame width and height
    var frames = this.createFramesFromImages_(images, framesInfo.getFrameWidth(), framesInfo.getFrameHeight());    
    var descriptor = new pskl.model.piskel.Descriptor('Imported piskel', '');
    
    //Add the new image to the first frame of the piskel project
    this.pController.addFrameAtCurrent(frames[0], 0); 

    var layers = this.pController.getLayers();
    var piskel = pskl.model.Piskel.fromLayers(layers, descriptor);
